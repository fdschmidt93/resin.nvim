...
function M.get_marks_positions()
  ...
  for bufnr, extmarks in pairs(M._marks) do
    ...
    for time, mark in pairs(extmarks) do
      bufnr_extmarks[time] = {
        begin_pos = api.nvim_buf_get_extmark_by_id(bufnr, resin_ns, mark.begin_extmark_id, {}),
        end_pos = api.nvim_buf_get_extmark_by_id(bufnr, resin_ns, mark.end_extmark_id, {}),
      }
    end
...
function M.get_text_by_id(bufnr, begin_extmark_id, end_extmark_id)
  local begin_pos = api.nvim_buf_get_extmark_by_id(bufnr, resin_ns, begin_extmark_id, {})
  local end_pos = api.nvim_buf_get_extmark_by_id(bufnr, resin_ns, end_extmark_id, {})
  local data = api.nvim_buf_get_text(bufnr, begin_pos[1], begin_pos[2], end_pos[1], end_pos[2] + 1, {})
  return data
end
